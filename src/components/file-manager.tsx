...
export default function FileManager() {
  ...
  const fetchFiles = async () => {
    try {
      ...
      const imageFiles = (Array.isArray(data) ? data : []).filter((f) =>
        isImageFile(f.Key)
      )
      ...
      await Promise.all(
        imageFiles.map(async (file) => {
          ...
          const { signedUrl } = img_data

          console.log(img_data, 'fetchFiles11')
          urlMap[file.Key] = signedUrl
        })
      )
      setImageUrls(urlMap)
    } catch (error) {
        ...
        <ul className="space-y-4">
          {files.map((file) => (
            ...
            >
              ...
              {/* {isImageFile(file.Key ?? '') && imageUrls[file.Key ?? ''] && (
                <>
